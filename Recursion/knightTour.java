...
public class knightTour {
    ...
    public static void printKnightsTour(int[][] chess, int r, int c, int upcomingMove) {
        if(r <0||c<0||r >= chess.length||c>= chess[0].length || chess[r][c] >0){
            return;
        }else if(upcomingMove == chess.length*chess.length){//when it reaches last move,make the move and display and reomove it futher to 
            // let other paths have their moves
