...
4. You are required to change the body of push function to accept the element even when the 
data array is completely full. In that case, you are required to reallocate the data array (to twice it's size). 
    ...
    public static class CustomStack {
        ...
        void push(int val) {
            if (tos == data.length - 1) {
                int [] ndata = new int[2* data.length];// create new array with double size of existing
                for(int i = 0; i< data.length;i++){// copy old array data to new one
                  ndata[i] = data[i];
                }
                data = ndata;// point new to old array
                tos++;
                data[tos] = val;
