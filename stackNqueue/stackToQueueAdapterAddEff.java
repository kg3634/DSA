/*1. You are required to complete the code of our StackToQueueAdapter class. The class should mimic the behaviour 
of a Queue and implement FIFO semantic.
  ...
  public static class StackToQueueAdapter {
    ...
    int remove() {
        if(size() == 0){
            ...
            while(size() > 1){// 1 tak check karenge as last element capture karna h
                helperS.push(mainS.pop());
            }

            int val = mainS.pop();//storing it as need to capture it and return

            while(helperS.size() > 0){// again will push all elment in main 
    ...
    int peek() {
        if(size() == 0){
            ...
            int val = mainS.pop();
            helperS.push(val);// not in remove as need to remove it as well
