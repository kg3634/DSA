/*1. You are required to complete the code of our TwoStack class. The class should implement LIFO behaviours for two 
...
2. Memory should be optimally used i.e. one of the stacks can use more elements in array but overflow should not happen before 
whole array is used. */
import java.io.*;
...
public class twoStackInArray {
  ...
  public static class TwoStack {
    ...
    int tos1;
    int tos2;
    //Logic:images/twoStackInArray.png
    // based on tos1(-1) and tos2(data.length), we will create all functions
    ...
    void push1(int val) {
      if(tos2 == tos1 +1){
          ...
          tos1++;// we have to first increament then only push value
          data[tos1] = val;
      }
    }
    ...
    void push2(int val) {
        if(tos2 == tos1 +1){
            ...
            tos2--;// we have to first decreament then only push
            data[tos2] = val;
        }        
    }
    ...
    int pop1() {
      if(size1() == 0){
          ...
          return -1;
      }else{// first capture value , then decrease
          int val = data[tos1];
          tos1--;
          return val;
      }
    ...
    int pop2() {
        if(size2() == 0){
            System.out.println("Stack underflow");
            return -1;
        }else{// first capture value , then increase
            int val = data[tos2];
            tos2++;
            return val;
        }    
