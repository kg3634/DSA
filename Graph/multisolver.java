...
public class multisolver {
   ...
   public static void multisolver(ArrayList<Edge>[] graph, int src, int dest, boolean[] visited, int criteria, int k, String psf, int wsf) {
    if(src == dest){
        ...
        //ceil : just largest: means cal smallest among all larger values than criteria
        if(wsf > criteria && wsf < cpathwt){
            cpathwt = wsf;
            cpath = psf;
        }
        //floor: just smallest: means cal largest among all smallest value than criteria
        if(wsf < criteria && wsf > fpathwt){
            fpathwt = wsf;
            fpath = psf;
        }

        //kth largest: using priority queue , selecting k best players in team, first slect any k teams members then compare and 
        // remove weak, so at the end best k players will be there in team, so in minimum periority queue, we will get smallest among k 
        //largest in peek that is out ans

        if(pq.size() < k){
            pq.add(new Pair(wsf,psf));//pair object is there to compare weight and provides smallest based on wt
        }else{
            if(wsf > pq.peek().wsf){
                pq.remove();
